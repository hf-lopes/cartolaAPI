SELECT CAST(SUM(
          CASE
            WHEN (team.id = match.visiting_team_id AND match.result = 'Visitante')
              THEN 3
            WHEN (match.result = 'Empate')
              THEN 1
            ELSE 0
            END) AS FLOAT) / {} as {}, team.name, team.id, s2.scout_id as scout_id
  FROM
(SELECT match.home_team_id as enemy_team, s1.s_year as s_year, s1.s_rodada as s_rodada, s1.scout_id as scout_id
          FROM
